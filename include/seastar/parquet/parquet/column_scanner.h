...
class PARQUET_TEMPLATE_CLASS_EXPORT TypedScanner : public Scanner {
 public:
  ...
  seastar::future<bool> NextLevels(int16_t* def_level, int16_t* rep_level) {
    return [=] {
      if (level_offset_ == levels_buffered_) {
        return typed_reader_->ReadBatch(
            static_cast<int>(batch_size_), def_levels_.data(),
            rep_levels_.data(), values_, &values_buffered_).then([=] (int64_t bytes_read) {
          levels_buffered_ = static_cast<int>(bytes_read);
          value_offset_ = 0;
          level_offset_ = 0;
          return seastar::make_ready_future<bool>(levels_buffered_ != 0);
        });
      }
      return seastar::make_ready_future<bool>(true);
    }().then([=] (bool levels_buffered) {
        if (!levels_buffered) {
          return false;
        }
        *def_level = descr()->max_definition_level() > 0 ? def_levels_[level_offset_] : 0;
