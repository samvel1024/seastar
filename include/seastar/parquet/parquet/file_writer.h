...
class PARQUET_EXPORT RowGroupWriter {
 public:
  ...
  struct Contents {
    ...
    // to be used only with ParquetFileWriter::AppendRowGroup
    virtual seastar::future<ColumnWriter*> NextColumn() = 0;
    // to be used only with ParquetFileWriter::AppendBufferedRowGroup
    virtual ColumnWriter* column(int i) = 0;
  ...
  /// directly written to the sink, once a new column is started, the contents
  /// of the previous one cannot be modified anymore.
  seastar::future<ColumnWriter*> NextColumn();
  /// Index of currently written column
  int current_column();
...
PARQUET_EXPORT
seastar::future<> WriteFileMetaData(const FileMetaData& file_metadata,
                       FutureOutputStream* sink);

PARQUET_EXPORT
seastar::future<> WriteMetaDataFile(const FileMetaData& file_metadata,
                       FutureOutputStream* sink);
...
class PARQUET_EXPORT ParquetFileWriter {
 public:
  ...
  struct Contents {
    ...
    virtual seastar::future<RowGroupWriter*> AppendRowGroup() = 0;
    virtual seastar::future<RowGroupWriter*> AppendBufferedRowGroup() = 0;
