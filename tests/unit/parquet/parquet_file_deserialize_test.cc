...
class TestPageSerde : public ::testing::Test {
 public:
  ...
  void InitSerializedPageReader(int64_t num_rows,
    ...
    auto stream = std::make_shared<seastarized::MemoryFutureInputStream>(out_buffer_);
    page_reader_ = seastarized::PageReader::Open(stream, num_rows, codec);
  }
 ...
 protected:
  ...
  std::unique_ptr<seastarized::PageReader> page_reader_;
...
TEST_F(TestPageSerde, DataPageV1) {
  ...
  std::shared_ptr<Page> current_page = page_reader_->NextPage().get0();
...
TEST_F(TestPageSerde, TestFailLargePageHeaders) {
  ...
  ASSERT_THROW(page_reader_->NextPage().get0(), ParquetException);
}
...
TEST_F(TestPageSerde, Compression) {
  ...
  for (auto codec_type : codec_types) {
    ...
    for (int i = 0; i < num_pages; ++i) {
      int data_size = static_cast<int>(faux_data[i].size());
      page = page_reader_->NextPage().get0();
...
class TestParquetFileReader : public ::testing::Test {
 public:
  void AssertInvalidFileThrows(const std::shared_ptr<Buffer>& buffer) {
    reader_.reset(new seastarized::ParquetFileReader());

    auto source = std::make_shared<seastarized::RandomAccessBuffer>(buffer);

    ASSERT_THROW(reader_->Open(seastarized::ParquetFileReader::Contents::Open(source).get0()),
                 ParquetException);
  }
