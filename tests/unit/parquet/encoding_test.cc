...
TEST(DictEncodingAdHoc, PutDictionaryPutIndices) {
  // Part of ARROW-3246
  auto dict_values = arrow::ArrayFromVector<arrow::BinaryType, std::string>(
      arrow::binary(), {"foo", "bar", "baz"});
  auto indices = arrow::ArrayFromVector<arrow::Int32Type>(arrow::int32(), {0, 1, 2});
  auto indices_nulls = arrow::ArrayFromVector<arrow::Int32Type>(
      arrow::int32(),
  ...
  auto expected = arrow::ArrayFromVector<arrow::BinaryType, std::string>(
      arrow::binary(),
      {true, true, true, false, true, true, false, true},
