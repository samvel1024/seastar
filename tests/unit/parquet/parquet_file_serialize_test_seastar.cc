...
class TestSerialize : public PrimitiveTypedTest<TestType> {
 ...
 protected:
  ...
  void FileSerializeTest(Compression::type codec_type) {
    ...
    auto file_writer = seastarized::ParquetFileWriter::Open(sink, gnode, writer_properties).get0();
    this->GenerateData(rows_per_rowgroup_);
    for (int rg = 0; rg < num_rowgroups_ / 2; ++rg) {
      seastarized::RowGroupWriter* row_group_writer;
      row_group_writer = file_writer->AppendRowGroup().get0();
      for (int col = 0; col < num_columns_; ++col) {
        auto column_writer =
            static_cast<seastarized::TypedColumnWriter<TestType>*>(row_group_writer->NextColumn().get0());
        column_writer->WriteBatch(rows_per_rowgroup_, this->def_levels_.data(), nullptr,
                                  this->values_ptr_).get();
        column_writer->Close().get();
      ...
      row_group_writer->Close().get();
    }
    // Write half BufferedRowGroups
    for (int rg = 0; rg < num_rowgroups_ / 2; ++rg) {
      seastarized::RowGroupWriter* row_group_writer;
      row_group_writer = file_writer->AppendBufferedRowGroup().get0();
      for (int batch = 0; batch < (rows_per_rowgroup_ / rows_per_batch_); ++batch) {
        for (int col = 0; col < num_columns_; ++col) {
          auto column_writer =
              static_cast<seastarized::TypedColumnWriter<TestType>*>(row_group_writer->column(col));
          column_writer->WriteBatch(
              rows_per_batch_, this->def_levels_.data() + (batch * rows_per_batch_),
              nullptr, this->values_ptr_ + (batch * rows_per_batch_)).get();
          // Ensure NextColumn() API which is specific to RowGroup cannot be called
          ASSERT_THROW(row_group_writer->NextColumn().get(), ParquetException);
        }
      }
      for (int col = 0; col < num_columns_; ++col) {
        auto column_writer =
  ...
  void UnequalNumRows(int64_t max_rows, const std::vector<int64_t> rows_per_column) {
    ...
    auto file_writer = seastarized::ParquetFileWriter::Open(sink, gnode, props).get0();
