...
#include "test_encryption_util.h"
#include "test_util.h"
...
using FileClass = seastarized::FileFutureOutputStream;
...
class TestEncryptionConfiguration : public ::testing::Test {
 ...
 protected:
  ...
  void EncryptFile(
    ...
    out_file = std::make_shared<seastarized::FileFutureOutputStream>(seastar::make_file_output_stream(out_f));

    // Create a ParquetFileWriter instance
    std::shared_ptr<parquet::seastarized::ParquetFileWriter> file_writer =
        parquet::seastarized::ParquetFileWriter::Open(out_file, schema_, writer_properties).get0();

    for (int r = 0; r < num_rgs; r++) {
      auto row_group_writer = file_writer->AppendRowGroup().get0();

      // Write the Bool column
      parquet::seastarized::BoolWriter* bool_writer =
          static_cast<parquet::seastarized::BoolWriter*>(row_group_writer->NextColumn().get0());
      for (int i = 0; i < rows_per_rowgroup_; i++) {
        bool value = ((i % 2) == 0) ? true : false;
        bool_writer->WriteBatch(1, nullptr, nullptr, &value).get();
      }
      ...
      for (int i = 0; i < rows_per_rowgroup_; i++) {
        int32_t value = i;
        int32_writer->WriteBatch(1, nullptr, nullptr, &value).get();
      }

      // Write the Int64 column. Each row has repeats twice.
      parquet::seastarized::Int64Writer* int64_writer =
          static_cast<parquet::seastarized::Int64Writer*>(row_group_writer->NextColumn().get0());
      for (int i = 0; i < 2 * rows_per_rowgroup_; i++) {
        ...
        int64_writer->WriteBatch(1, &definition_level, &repetition_level, &value).get();
      }

      // Write the INT96 column.
      parquet::seastarized::Int96Writer* int96_writer =
          static_cast<parquet::seastarized::Int96Writer*>(row_group_writer->NextColumn().get0());
      for (int i = 0; i < rows_per_rowgroup_; i++) {
        parquet::Int96 value;
        value.value[0] = i;
        value.value[1] = i + 1;
        value.value[2] = i + 2;
        int96_writer->WriteBatch(1, nullptr, nullptr, &value).get();
      }

      // Write the Float column
      parquet::seastarized::FloatWriter* float_writer =
          static_cast<parquet::seastarized::FloatWriter*>(row_group_writer->NextColumn().get0());
      for (int i = 0; i < rows_per_rowgroup_; i++) {
        float value = static_cast<float>(i) * 1.1f;
        float_writer->WriteBatch(1, nullptr, nullptr, &value).get();
      }

      // Write the Double column
      parquet::seastarized::DoubleWriter* double_writer =
          static_cast<parquet::seastarized::DoubleWriter*>(row_group_writer->NextColumn().get0());
      for (int i = 0; i < rows_per_rowgroup_; i++) {
        double value = i * 1.1111111;
        double_writer->WriteBatch(1, nullptr, nullptr, &value).get();
      }

      // Write the ByteArray column. Make every alternate values NULL
      parquet::seastarized::ByteArrayWriter* ba_writer =
          static_cast<parquet::seastarized::ByteArrayWriter*>(row_group_writer->NextColumn().get0());
      for (int i = 0; i < rows_per_rowgroup_; i++) {
        ...
        if (i % 2 == 0) {
          ...
          ba_writer->WriteBatch(1, &definition_level, nullptr, &value).get();
        } else {
          int16_t definition_level = 0;
          ba_writer->WriteBatch(1, &definition_level, nullptr, nullptr).get();
        }
      }

      // Write the FixedLengthByteArray column
      parquet::seastarized::FixedLenByteArrayWriter* flba_writer =
          static_cast<parquet::seastarized::FixedLenByteArrayWriter*>(row_group_writer->NextColumn().get0());
      for (int i = 0; i < rows_per_rowgroup_; i++) {
        parquet::FixedLenByteArray value;
        char v = static_cast<char>(i);
        char flba[kFixedLength] = {v, v, v, v, v, v, v, v, v, v};
        value.ptr = reinterpret_cast<const uint8_t*>(&flba[0]);
        flba_writer->WriteBatch(1, nullptr, nullptr, &value).get();
      }
    }
